...
```{r}
# 2 
Garinger.Ozone <- rbind(GaringerOzone2010, GaringerOzone2011, GaringerOzone2012, GaringerOzone2013, GaringerOzone2014, GaringerOzone2015, GaringerOzone2016, GaringerOzone2017, GaringerOzone2018, GaringerOzone2019)

# 3
Garinger.Ozone$Date <- as.Date(Garinger.Ozone$Date, format = "%m/%d/%Y")
class(Garinger.Ozone$Date)

# 4
Garinger.Ozone.clean <- Garinger.Ozone %>%
...
# 6
Garinger.Ozone <- left_join(Days, Garinger.Ozone.clean)
...
```{r}
Ozone.Time <- ggplot(Garinger.Ozone, 
...
> Answer: Daily data is best interpolated as a linear function because the day before a missing data point is often a good indicator for the following day.
...
> Answer: The seasonal Mann-Kendall accounts for the seasonality of the years without assuming a normality of the data, and the time series is equally spaced (first of every month), which is needed for the SMK. 
...
```{r}
# 8
Garinger.Ozone$Daily.Max.8.hour.Ozone.Concentration <- na.approx(Garinger.Ozone$Daily.Max.8.hour.Ozone.Concentration)

Garinger.Ozone$DAILY_AQI_VALUE <- na.approx(Garinger.Ozone$Daily.Max.8.hour.Ozone.Concentration)

summary(Garinger.Ozone$Daily.Max.8.hour.Ozone.Concentration)

# 9
GaringerOzone.monthly <- Garinger.Ozone %>%
...
# 13
Ozone.Time <- ggplot(GaringerOzone.monthly, aes(y = Daily.Max.8.hour.Ozone.Concentration, x = Date)) +
  geom_line() +
  geom_point() +
  labs(x = "Date", y = "Average Monthly Ozone Concentration (ppm)")
print(Ozone.Time)
...
> Answer: Ozone concentrations have decreased signficantly at the stations from 2010 to 2019 (Seasonal Mann-Kendall, z = -1.963, p-value < 0.05).
