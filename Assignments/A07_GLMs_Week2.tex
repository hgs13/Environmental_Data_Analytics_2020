...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{chem.phys.raw <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"}\NormalTok{)}
...
\CommentTok{#install.packages("harrypotter")}
\KeywordTok{library}\NormalTok{(harrypotter)}
\NormalTok{pal <-}\StringTok{ }\KeywordTok{hp}\NormalTok{(}\DataTypeTok{n =} \DecValTok{9}\NormalTok{, }\DataTypeTok{house =} \StringTok{"Ravenclaw"}\NormalTok{)}
\NormalTok{pal}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## [1] "#006699FF" "#1479ACFF" "#2E93C6FF" "#56ADD9FF" "#98C2D9FF" "#C8B197FF"
## [7] "#D99451FF" "#CF7318FF" "#B35900FF"
\end{verbatim}
...
\begin{Highlighting}[]
\CommentTok{#3}
\NormalTok{chem.phys.clean <-}\StringTok{ }\NormalTok{chem.phys.raw }\OperatorTok{%>%}
...
\CommentTok{#4}
\NormalTok{chem.phys.AIC <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ chem.phys.clean, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
\KeywordTok{step}\NormalTok{(chem.phys.AIC)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ year4 + daynum + depth, data = chem.phys.clean)
## 
## Coefficients:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{chem.phys.regression <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ chem.phys.clean, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
\KeywordTok{summary}\NormalTok{(chem.phys.regression)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: The day, the year, and the depth of the water significantly
affect temperature in the lakes (linear regression, R\^{}2 = 0.7417,
p-value \textless{} 0.01). The model explains 74.17\% of the observed
variance.
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{#6}
\NormalTok{chem.phys.interaction <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ chem.phys.clean, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{lakename }\OperatorTok{*}\StringTok{ }\NormalTok{depth)}
\KeywordTok{summary}\NormalTok{(chem.phys.interaction)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ lakename * depth, data = chem.phys.clean)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
...
\begin{quote}
Answer: The is a signifcant interaction between depth and lakename
(ANCOVA, R\^{}2 = 0.7861, p-value \textless{} 0.0001). The depth and
lakename account for 78.61\% of the variance in temperature.
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{#8}
\NormalTok{temp.depth <-}\StringTok{ }
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(chem.phys.clean, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ depth, }\DataTypeTok{y =}\NormalTok{ temperature_C, }\DataTypeTok{color =}\NormalTok{ lakename)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\DataTypeTok{alpha =} \DecValTok{50}\OperatorTok{/}\DecValTok{100}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale_color_manual}\NormalTok{(}\DataTypeTok{values =}\NormalTok{ pal) }\OperatorTok{+}
...
\StringTok{  }\KeywordTok{coord_cartesian}\NormalTok{(}\DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{35}\NormalTok{)) }\OperatorTok{+}
...
\begin{quote}
Answer: 2.1 \% because 0.267 (conditional) - 0.246 (marginal) = 0.021,
which is 2.1\%
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{#9d}
\NormalTok{litter.anova <-}\StringTok{ }\KeywordTok{anova}\NormalTok{(litter.mixed, litter.fixed)}
\KeywordTok{summary}\NormalTok{(litter.anova)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                                                                                                             call  
##  gls(model = dryMass ~ nlcdClass * functionalGroup, data = litter.processed)                                  :1  
##  lme.formula(fixed = dryMass ~ nlcdClass * functionalGroup, data = litter.processed,     random = ~1 | plotID):1  
##                                                                                                                   
##                                                                                                                   
##                                                                                                                   
##                                                                                                                   
...
##  Min.   :1.00   Min.   :25.00   Min.   :9039   Min.   :9179   Min.   :-4504  
##  1st Qu.:1.25   1st Qu.:25.25   1st Qu.:9043   1st Qu.:9183   1st Qu.:-4501  
##  Median :1.50   Median :25.50   Median :9048   Median :9187   Median :-4499  
##  Mean   :1.50   Mean   :25.50   Mean   :9048   Mean   :9187   Mean   :-4499  
##  3rd Qu.:1.75   3rd Qu.:25.75   3rd Qu.:9053   3rd Qu.:9190   3rd Qu.:-4496  
##  Max.   :2.00   Max.   :26.00   Max.   :9058   Max.   :9194   Max.   :-4493  
##                                                                              
##      Test      L.Ratio         p-value       
##        :1   Min.   :21.51   Min.   :3.5e-06  
##  1 vs 2:1   1st Qu.:21.51   1st Qu.:3.5e-06  
##             Median :21.51   Median :3.5e-06  
##             Mean   :21.51   Mean   :3.5e-06  
##             3rd Qu.:21.51   3rd Qu.:3.5e-06  
##             Max.   :21.51   Max.   :3.5e-06  
##             NA's   :1       NA's   :1
\end{verbatim}
...
\begin{quote}
Answer: The fixed effects model is a better model than the mixed effects
model because the AIC is lower for the fixed effects model in the anova
