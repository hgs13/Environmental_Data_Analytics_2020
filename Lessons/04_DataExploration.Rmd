...
#if use this code for working directory, only works on one computer; not transferrable
#can add subfolder with this
```
...
```{r, message = FALSE}
# Load package
#message = FALSE means r will still load it but won't cause trouble on knitting when working with loading tidy verse (check marks are problem children because not asky text); LaTeX or asky text error message
library(tidyverse)
```
...
```{r}
# csv = comma serpated values, can go into Excel with this file
...
# Period means go one folder in and do these things with subfolder
# Always need quotes around it
# If not working, change knit directory to project directory from document directory
# Can navigate from Finder then right click while pressing command to copy pathname
...
# double period means go out one folder
# read.csv just reads it and puts it out
...
> ANSWER:  This file can be found under USGSUSGS_Site02085000_Flow_Raw.csv in the metadata folder. Metadata is important for clarity of names and abbreviations. Each naming conventions is thoroughly outlined with units and what each title means. There is enough detail to find exactly which data is needed for each analyses you are wanting to do. The pipeline and source information steps are useful for troubleshooting and finding the original data. The naming convention is important for the consistency of naming the files, especially when working with other people.
...
colnames(USGS.flow.data) #hey R, what are the column names? not helpful! so rename (have to rename all of them this way!)
#UNLESS: to rename one, insert [] and say which column with matrix subsetting

# Rename columns
# never go beyond 80 characters per line (or thereabouts)
# only changing data frame in R, not Excel
# to change in everything, write code to save data frame as something new (see end)
...
str(USGS.flow.data) #structure of dataframe
dim(USGS.flow.data) #dimensions of dataframe
length(USGS.flow.data) #gives number of columns
nrow(USGS.flow.data) #gives number of rows

head(USGS.flow.data) #gives first 6 rows by default
head(USGS.flow.data, 10) #for specific number of rows, add ", number"
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #rows 30000 to 30005, columns 3, 8, and 14

class(USGS.flow.data$datetime) #ask for specific columns
...
> Answer: They are now noted as "NA". The ones that still appear as blanks appear as a "Factor" or "Character". Numeric cells that are blank appear as "NA".

## Adjusting Datasets

### Removing NAs

Notice in our dataset that our discharge and gage height observations have many NAs, meaning no measurement was recorded for a specific day. In some cases, it might be in our best interest to remove NAs from a dataset. Removing NAs or not will depend on your research question. The cells that were once NAs will be gone.
...
> Answer: It would be helpful to know when NA exists for future data collection. NA might be irrelevant when comparing mean and such, so you want to remove NAs.

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #take out ANY row that contains an NA; can be dangerous if just wanting to examine just certain columns/data
dim(USGS.flow.data)
...
In some cases when dates are provided as integers, you may need to provide an origin for your dates. Beware: the "origin" date for Excel (Windows), Excel (Mac), R, and MATLAB all have different origin dates. Google this if it comes up. The origin date for R is January 1, 1970.
...
```{r}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") #tell R what the format is for the data right now, not what we want it to be
...
```{r}
write.csv(USGS.flow.data, file = "./Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE) #yay, now it works in Excel too
