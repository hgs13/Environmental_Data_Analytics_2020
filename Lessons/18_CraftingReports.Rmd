...
* Make a bullet list with an asterisks then space!
* *italic*
* **bold**
* **Documentation** of data pipelines steps
* Store *code*, *visualization*, and *text* in one place
* *Organize* code into sections
* Aesthetically pleasing
* Manage change in a *reproducible* fashion
...
nutrients.processed <- nutrients %>%
  ...
  filter(depth == "0")
summary(nutrients.processed$tn_ug)
```
...
tn.summary <-  nutrients.processed %>%
  filter(!is.na(tn_ug) & !is.na(tp_ug)) %>%
  group_by(lakename) %>%
  summarise(meanTN = mean(tn_ug), 
            sdTN = sd(tn_ug), 
            minTN = min(tn_ug),
            maxTN = max(tn_ug))
tp.summary <- nutrients.processed %>%
  group_by(lakename) %>%
  summarize(meanTP = mean(tp_ug, na.rm = FALSE), 
            sdTP = sd(tp_ug, na.rm = FALSE),
            minTP = min(tp_ug, na.rm = FALSE),
            maxTP = max(tp_ug, na.rm = FALSE))
```
