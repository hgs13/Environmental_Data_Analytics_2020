...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #lty = line type, 2 = dashed line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #added aestethic after hline because ggplot adds by layers, so points will now go BEHIND line, alpha = .5 is 50% transparency, size is size of point
  ...
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1,
            fontface = "bold") + #x is where text is, y is where label is, hjust 1 = justified right, hjust = -1 = justified left
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #x value range/bounds
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + #expression = anything in parenthesis is written as is,[] means super or subscript
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #if labels overlap, turn them 45 degrees
print(O3plot7)  
...
NvsP2 <-
  ...
  scale_color_viridis(option = "magma", direction = -1, end = 0.8) + #end chops of bottom 20% or whatever of color scales for less pale yellows or bottom color
...
# place your additional edits here
  labs(x = NULL, y = "Concentration (ug/L)",
       color = "Nutrient") +
