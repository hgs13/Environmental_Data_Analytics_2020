...
shapiro.test((EPAair.subsample$Ozone)) #we can say the p value is signficantly small that our data is not skewed?
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) #semicolon is running two lines of coade at the same time; qqnorm shoes what it is, qqline is what it should theoretically look like

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample #95% confidence

Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  #geom_density(stat = "count", fill = "gray") + #shows actual count of data
  geom_density(fill = "gray") +
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #this means to have axis actually start at 0
print(Ozone.plot) #majority of our data lie at less than 50, which supports our t-test
```
...
> EPA ozone measurements for 2018-2019 were signficantly lower than 50, the AQI threshold for "good" air quality (t-test; t=-57.98, p<0.0001). (Do not put null hypothesis or alternative hypothesis, want general reader to understand the sentence) 
...
```{r}
shapiro.test(EPAair$Ozone[EPAair$Year == 2018]) #were ozone levels different between years?
shapiro.test(EPAair$Ozone[EPAair$Year == 2019])
var.test(EPAair$Ozone ~ EPAair$Year) #test whether variance is similar

ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly() #similar distribution

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year) #instead of comparing to a mu, compare the years, alternative means is that one is different from the other
O3.twosample
O3.twosample$p.value

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Year) #treating year as contiunous response variable
summary(O3.twosample2) #for a linear model, need to call it as a summary, slight rounding error due to underlying equation

plot(O3.twosample2) #plot function can only call lm things, not t.test
par(mfrow = c(2,2)) #any time I plot a plot, I want you to plot 4 in 2 by 2; must reset to 1,1 to have them one at a time
```
...
```{r}
O3.onesample.wilcox <- wilcox.test(EPAair$Ozone, mu = 50, alternative = "less") #gives us less statistical robustness
O3.onesample.wilcox
...
```{r}
#1 
O3.year.density <- ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_density(stat = "count", fill = "gray") +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))
print(O3.year.density)

#2
O3.year.boxplot <- ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_boxplot(stat = "count", fill = "gray") +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) 
print(O3.year.boxplot)  

#3
O3.year.violin <- ggplot(EPAair, aes(x = Year, y = Ozone, color = Year)) +
  geom_violin()
print(O3.year.violin)
```
