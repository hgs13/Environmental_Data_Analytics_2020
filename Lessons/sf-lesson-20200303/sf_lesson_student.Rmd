...
here("data", "tabular_data", "conterm_basinid.txt") #go to data folder then go to tabular data then go to conterm file, not doing anything with it, just creating a relative file path
...
>Maps package also good for all this stuff
...
# note how I'm using the here package!, st_read is reading in a shp file
```
...
my_tabular_data_raw <- read.csv(here("data", "tabular_data", "conterm_hydro.txt"))
...
gages_tabular_as_sf <- st_as_sf(gages_tabular, coords = c("LNG_GAGE", "LAT_GAGE"), crs = 4326, dim = "XY") # lat long so set CRS = WGS84 #take this data frame, set coordinates
...
# add your code here
NC_water <- select(southeast_state_bounds_raw$)
# check
# add your code here
