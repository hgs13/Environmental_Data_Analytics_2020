...
* **Autocorrelation**: Data points are not independent from one another (i.e., the measurement at a given time point is dependent on previous time point(s)). Temperature today indicates temperature tomorrow.
...
* **Heteroscedasticity**: The variance of the time series is not constant over time. Getting more variable or less variable as time goes on.
...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean) #just drop NAs from the discharge.mean column

EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01")) #nothing between 1982 and 1985 after dropping na

EnoDischarge.late <- EnoDischarge.complete %>%
  filter(datetime > as.Date("1985-01-01")) #when filtering, still have to tell it's a date for some reason
```
...
Note: time series objects must be equispaced. In our case, we have daily data with no NAs in the data frame, so we don't need to worry about this. We will cover how to address data that are not equispaced later in the lesson. It won't preserve the date information for us.

```{r}
EnoDischarge.late_ts <- ts(EnoDischarge.late[[8]], frequency = 365) #[[]] allows selection for one column (instead of $discharge.mean), frequency = 365 makes a dailytime series (consider leap year if that would impact you)
```
...
```{r}
?stl
# Generate the decomposition, look at the help if you don't understand, must be a time series object not a data frame; if periodic can find it for you, if weekly, specify 7 for a week or 12 for monthly; can run with what we've given it below
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic")

# Visualize the decomposed series.
# x axis is 35 years, shows how many itertions there are of your cycle
# seasonal + trend + remainer = data
# seasonal + trend are not extremely representative of data, so remainder looks similar to data
# there's not a strong seasonal trend, so the same thing is just repeated over 35 years; seasonal shows every cycle we have specified
# trend is a moving average
plot(EnoDischarge.late_Decomposed)

# We can extract the components and turn them into data frames, 3 columns in time.series correspond to seasonal, trend, and remainder; [all of the rows, columns 1 - 3]
...
# Visualize how the trend maps onto the data, specify aes in each on because want different aes for each componenet, trend shows when there's an increase or decrease for extended periods of time
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff", size = 1) +
  geom_hline(yintercept = 0, lty = 2) + # helpful to show y-axis doesn't start at 0
  ylab(expression("Discharge (ft"^3*"/s)"))

# Visualize how the seasonal cycle maps onto the data, decomposition can go below 0, so be careful
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
* **Change point detection**, e.g., `pettitt.test` (package: trend), `breakpoints` (package: strucchange), `chngpt.test` (package: chngpt), multiple tests in package: changepoint. useful when you don't know where a slope changes, but R  will find it for you
...
```{r}
# assumptions for t-test: normal distribution, equal variance across two different datasets
EnoDischarge.early.subsample <- sample_n(EnoDischarge.early, 5000) #shapiro can only run 5000, so this is just a random 5000 numbers
EnoDischarge.late.subsample <- sample_n(EnoDischarge.late, 5000)

shapiro.test(EnoDischarge.early.subsample$discharge.mean) #test for normal distribution, not well supported that we would be able to run a t-test 
shapiro.test(EnoDischarge.late.subsample$discharge.mean)

var.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean) #test for equal variance

wilcox.test(EnoDischarge.early$discharge.mean, EnoDischarge.late$discharge.mean) #our two different periods are signifcantly different
...
> The mean daily discharge from the Eno River was significantly higher in the early period of data collection (1927-1971) than the late period of data collection (1985-2020) (Wilcoxon test, W = 1.17*10^8, p < 0.0001). Whether this is from different collection methods or envirnonmental change is unclear. 
> Represent this graphically with boxplot or line plot between the two of the daily mean discharge. May have to do some creative wrangling or plotting because two different data frames.
...
> aggregation: if you have too much data, combine with summary statistics

> subsampling: if too much data, take a certain subset (like the first day of the month)

> interpolation: fill in gaps from existing data by extracting the trend from them

Common interpolation methods: (see notebook on 2/27 to see the graph example), which on you use depends on previous knowledge of data
...
print(NCOzone) #not great to interpolate across these because it seems the low point is during the time we are missing
...
summary(NCAir$Site.Name) #how many rows in each column, Garinger is a good place because it is only missing 8 data points
...
NCAir.Garinger$Ozone <- na.approx(NCAir.Garinger$Ozone) #new column for ozone is an na.approx 
...
* **Mann-Kendall**: no seasonality, non-parametric, no temporal autocorrelation (the further away you get in time, the less correlated the data are), missing data allowed. Function: `mk.test` (package: trend)
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution (be equally spaced). Function: `smk.test` (package: trend)
...
> The mean accounts for some variablilty in the data across the month and includes the outliers moreso than the median.
...
EnoDischarge.late.monthly <- EnoDischarge.late %>%
  ...
  group_by(Year, Month) %>%
  summarise(Discharge = mean(discharge.mean)) #if use sum, realize the months have different days (sum does better with year)
#new columns will be whatever is in "group_by" and "summarise"

#we are "creating" a date so we can graph it, but we won't use it in the analysis #use this data fram with the new column($), paste what is in the year and month column, make a one in the new column, and separate with a dash, then turn it into a date 
# could feasibly do a pipe function with mutate and the same function
...
# frequency should come back around every 12 rows, include frequency and start and end dates will not be included in the time series, so included it manually?
EnoDischarge.late.monthly.ts <- ts(EnoDischarge.late.monthly$Discharge, frequency = 12, 
                        start = c(1985, 10, 1), end = c(2019, 12, 1))
# Run SMK test (must be run on an ts)
EnoDischarge.late.trend <- smk.test(EnoDischarge.late.monthly.ts)

# Inspect results
EnoDischarge.late.trend
#the smk test will say if there's a monotonic trend and if it's positive or negative, z-score shows we have a slightly decreasing trend but it is not signficant (p-value is very high)
...
#there's a lot of variability with this data and no significant trends, there could be trends over "seasons" but not in the data overall
```

What would we conclude based on these findings? 

> There is no signficant change in the mean flow yearly or monthly discharge of the Eno River from 1985 to present (Seasonal Mann-Kendall, z = -0.13967, p-value > 0.05). 
