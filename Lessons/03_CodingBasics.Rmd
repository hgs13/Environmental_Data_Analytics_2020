...
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #dimnames name rows first then columns
...
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # "I want r to make a matrix called 'matrix4' from the matrix cells with 2 row & 2 columns and be named by rows"
matrix4
matrix3
...
e <- c("red", "white", "red", NA) # What type of vector? character; insert "NA" so number of item under each letter matches up, don't need quotes around it
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? logical
dataframe1 <- data.frame(d,e,f) #data.frame creates a data frame; in parenthesis just shows what you what to include
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #c means list of 3 things to use; gives title/name to each column in dataframe

dataframe1[1:2,] #row is first number; first row should be names, next row should be data
dataframe1[c("ID","Passed")] # 
dataframe1$ID #dollar sign means call up this specific portion of it

```
Question: How do the different types of data appear in the Environment tab?

> Answer: The data appear in the Environment tab grouped by types of vector.

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: It asks/ tries to autofill the 3 columns from the data frame.
...
> Answer: It will attach the packages and show a nice graph of what's been loaded.
