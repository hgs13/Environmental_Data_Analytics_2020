...
getwd()
library(plyr)
#want to always load plyr before dplyr because functions in dplyr will mask plyr function (which we want)
library(tidyverse)
library(lubridate)
...
str(NTL.phys.data.PeterPaul) #factor = any alphabet, int = all whole number, num = decimals
...
NTL.nutrient.data.PeterPaul <- 
  ...
  droplevels(NTL.nutrient.data.PeterPaul)
...
# 0 indicates count
# drawback of filter function, they're still there
# drop level completely removes the other lake names
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul)
#fixed it! Can also use pipe like above
summary(NTL.nutrient.data.PeterPaul$lakename)
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #pull from this data set
  mutate(month = month(sampledate)) %>% #pull month from sampledate column
  select(lakeid:daynum, month, sampledate:comments) %>% # 
  drop_na(depth) #drop NAs from specific column
...
```{r}
# Gather nutrient data into one column, make dataset long format
# preserves NAs
...
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered,
                                               !is.na(concentration))
#actually removes from data set
...
# Spread nutrient data into separate columns, supply which column you want to gather from and where to put it
...
# does not preserve original column
# Opposite of 'separate' is 'unite'
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, sampledate,
                                              c("Y", "m", "d"))
...
```{r}
# join, combine in mostly horizontal way
...
```{r}
#point to specific file, then look for pattern where they area call * (something).csv, and look for the full names of them
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles

Litter <- LitterFiles %>%
  ldply(read.csv)
#read anything that's a .csv from the list of LitterFiles
```
...
#can tell R which ones to joing by
dim(Litter)
dim(Trap)
dim(LitterTrap)

#have same number of rows, difference in columns means added the 20 different columns from trap and joined the 3 that were the same into one
...
```{r}
#lets take data frame, then only want surface data (depth ==0), then show each lake at a certain month, then filter out NAs for each of the things listed, then create new columns
#this is where dplyr vs plyr makes a difference
