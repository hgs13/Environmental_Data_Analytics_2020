...
#making sure R recognizes date "%Y = 4 digit year DASH %m = 2 digit month DASH %d = 2 digit day"
...
```{r, fig.height = 3, fig.width = 4}
# plus at end of line instead of comma is syntax of ggplot
...
# here's your bins, do what you want with it
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# limits say start at 0 and end at 500
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# to save in environment, use assignment operator, but won't give it to you in your plot tab
# to show, use print function 
...
#
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) + #for one column of data set
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "darkgray") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 2) +
  scale_x_continuous(limits = c(0, 10))

#use summary functions or other essential functions to see why peaks may be different (number of NAs and such)  
summary(USGS.flow.data$gage.height.max)
summary(USGS.flow.data$gage.height.min)
summary(USGS.flow.data$gage.height.mean)

# works for factor vectors to show differences and add legend
ggplot(USGS.flow.data) +
